(function (root, factory) {
  ...
  function calculateWeeklyFocus({ favorites = [], notes = [], questions = [] } = {}, options = {}) {
    ...
    const questionMap = questions instanceof Map
      ? questions
      : new Map(questions.map((question) => [question.id, question]));
    const counts = new Map();

    function include(row, timestampField, source) {
      ...
      if (!question) return;
      const seen = new Set();
      FOCUS_TAG_FIELDS.forEach((field) => {
        (question[field] || []).forEach((rawTag) => {
          const tag = String(rawTag || "").trim();
          if (!tag || seen.has(tag)) return;
          seen.add(tag);
          const current = counts.get(tag) || { tag, field, count: 0, latestAt: 0, sources: new Set() };
          current.count += 1;
          current.latestAt = Math.max(current.latestAt, timestamp);
    ...
    favorites.forEach((row) => include(row, "created_at", "favorite"));
    notes.forEach((row) => include(row, "updated_at", "note"));
    const ranked = [...counts.values()]
      .sort((a, b) => b.count - a.count || b.latestAt - a.latestAt || a.tag.localeCompare(b.tag, "zh-CN"))
      .slice(0, limit);
    const maximum = ranked[0]?.count || 0;
    return ranked.map(({ sources, ...item }) => ({
      ...item,
      share: maximum ? Math.round(item.count / maximum * 100) : 0,
